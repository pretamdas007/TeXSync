...
\usepackage{amsmath, amssymb, amsfonts, physics, tensor}
\usepackage{tikz}
...
\draw[very thin, gray] (-0.5,-0.5) grid (3.5,5);
\draw[thick,->] (-0.5,0) -- (3.5,0) node[right] {$x$};
\draw[thick,->] (0,-0.5) -- (0,5) node[above] {$y$};
\draw[blue, ultra thick, domain=0:2.8, samples=100] plot (\x, {0.5*\x*\x});
\node at (2.2,3.5) {\color{blue}$y = \frac{1}{2}x^2$};
\end{tikzpicture}
\end{center}
